...
local function menu()
    if menu_elements.tree_tab:push("Evade") then
        ...
        if menu_elements.main_boolean:get() then
            menu_manager.render_targeting_dropdown("evade", menu_elements.targeting_mode)
...
local function logics(target)
    ...
    if cast_spell.position(spell_data.evade.spell_id, target_position, 0) then
        local targeting_options = menu_manager.get_targeting_options_for_spell("evade")
        console.print("Cast Evade - Target: " ..
            targeting_options[menu_elements.targeting_mode:get() + 1] .. ", Mobility Only: " ..
...
local function out_of_combat()
    ...
    -- check if we are in a combat area (not in town/safe zone)
    local local_player = get_local_player()
    local in_combat_area = my_utility.is_buff_active(local_player, spell_data.in_combat_area.spell_id,
        spell_data.in_combat_area.buff_id);
    if not in_combat_area then return false, 0 end;
    ...
    -- if standing still
    if not is_moving then return false, 0 end;
    ...
    if is_dashing then return false, 0 end;
